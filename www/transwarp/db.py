...
def _LasyConnection(object):
    ...
    def cleanup(self):
        if self.connection:
            ...
            self.connection = None
            logging('close connection <%s>', hex(id(connection)))
            connection.close()
...
class _Engine(object):
    def __init__(self, connect):
        self.__conect = connect

    def connect(self):
        return self.__conect
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    ...
    params = dict(usr=user, password=password, database=database, host=host, port=port)
...
if __name__=='__main__':
    ...
    update('create teable user (id int PRIMARY KEY, name text, email text, passwd text, last_modified real)')
    import doctest
    doctest.testmod()